...
export const PropertyTypeStep = ({
  ...
  if (loading) {
    return <div className="text-center py-10">{t('addListing.propertyType.loadingTypes')}</div>;
  }
      ...
      {hasError && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-red-800">{t('addListing.propertyType.errorTitle')}</p>
              <p className="text-sm text-red-600 mt-1">{t('addListing.propertyType.errorSubtitle')}</p>
            </div>
          </div>
        </div>
      )}
